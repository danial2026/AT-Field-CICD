#!/bin/bash
# =============================================================================
# TEMPLATE: Docker build & deploy - PRODUCTION
# =============================================================================
...
#   IMAGE_TAG     tag for the built image (commit SHA is a good default)
...
#   - For production prefer immutable tags (commit SHA) over `latest`.
# -----------------------------------------------------------------------------
set -euo pipefail

PROJECT_DIR="/srv/${CI_REPO:-myapp}/prod"
COMPOSE_FILE="docker-compose.yml"
SERVICE_NAME="app"
IMAGE_TAG="${CI_COMMIT:-latest}"
GIT_BRANCH="main"
REPO_URL="${CI_CLONE_AUTH_URL:-${CI_CLONE_URL:-}}"

info() { echo "[build-prod-docker] $*"; }
die()  { echo "[build-prod-docker] ERROR: $*" >&2; exit 1; }

echo "==============================="
echo " BUILD_PROD_DOCKER ($(date -u +%FT%TZ))"
echo "==============================="
...
echo "==============================="
echo " BUILD_PROD_DOCKER SKELETON READY"

...
function getRunMetaByLogFiles(logFiles) {
  if (!logFiles.length) return {};
  const placeholders = logFiles.map(() => '?').join(',');
  const rows = getDb().prepare(`
    SELECT log_file, id AS run_id, repo_name, keyword, status
    FROM job_runs WHERE log_file IN (${placeholders})
  `).all(...logFiles);
  const map = {};
  for (const r of rows) map[r.log_file] = r;
  return map;
}
...
function createNotification({ userId, name, type, config, events = [], enabled = 1 }) {
  ...
  return getNotification(info.lastInsertRowid);
}
...
function updateNotification(id, fields) {
  ...
  if (!sets.length) return getNotification(id);
  sets.push('updated_at = ?');
  vals.push(now(), id);
  getDb().prepare(`UPDATE notifications SET ${sets.join(', ')} WHERE id = ?`).run(...vals);
